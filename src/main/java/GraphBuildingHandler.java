...
public class GraphBuildingHandler extends DefaultHandler {
    ...
    private LinkedList<Long> nodes = new LinkedList<>();
...
//            System.out.println("GraphDB.Node's name: " + attributes.getValue("v"));
        }
    }
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException {
        if (qName.equals("way")) {
            ...
            if (isValidWay) {
                for (long i : nodes) {
                    g.getNode(i).isWay = true;
                    g.getNode(i).way = wayName;
                }
                for (int i = 0; i < nodes.size()-1; i++) {
                    g.addEdge(nodes.get(i), nodes.get(i + 1), wayName);
                }
            }
        }
